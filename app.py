...
def olx_teste_auto():
    ...
    threads_response = olx_get("https://www.olx.pt/api/partner/threads")
    if not threads_response.ok:
        return {
            "erro": "Erro ao obter conversas do OLX",
            "codigo_olx": threads_response.status_code,
            "resposta_olx": threads_response.text,
        }, 500

    threads = threads_response.json().get("data", [])
    mensagens_encontradas = []

    # Percorre TODAS as conversas devolvidas pela API, independentemente do anúncio.
    ...
    for mensagem in mensagens_encontradas:
        ...
        if any(p in texto for p in [
            ...
            resposta = f"{saudacao} Sim, a viatura continua disponível. Onde podemos ajudar?"
            ...
            resposta = (
                ...
                "ano, quilometragem e motorização para o WhatsApp 962 148 367 e fazemos uma avaliação."
            )
            ...
            resposta = (
                f"{saudacao} De momento estamos a atualizar as nossas soluções de financiamento, "
                "pelo que temporariamente não estamos a realizar novos processos. "
                "Prevemos voltar a disponibilizar esta opção em breve."
            )

        elif any(p in texto for p in [
            "onde estão", "onde estao", "onde fica", "localização", "localizacao",
            "morada", "onde posso ver", "onde ver", "onde têm os carros", "onde tem os carros"
        ]):
            resposta = (
        ...
        elif "garantia" in texto:
            resposta = (
                f"{saudacao} As condições de garantia dependem da viatura e das condições da venda. "
                "Quando aplicável, trabalhamos com garantia até 18 meses. "
                "Podemos confirmar as condições específicas desta viatura."
            )
            ...
            resposta = (
                f"{saudacao} Claro. Podemos combinar uma visita para ver a viatura e esclarecer "
                "todas as questões. Indique-nos, por favor, o dia e horário que lhe dão mais jeito."
            )

        elif any(p in texto for p in ["mais fotos", "mais fotografias", "fotos", "fotografias", "vídeo", "video"]):
            resposta = (
                f"{saudacao} Claro. Podemos enviar mais fotografias ou vídeos da viatura. "
                "Diga-nos que detalhes pretende ver ou contacte-nos pelo WhatsApp 962 148 367."
            )

        elif any(p in texto for p in ["quilómetros", "quilometros", "km", "quantos kms", "quantos km"]):
            resposta = (
                f"{saudacao} A quilometragem encontra-se indicada no anúncio. "
                "Se tiver alguma questão específica sobre o histórico da viatura, podemos esclarecer."
            )
            ...
            resposta = (
                f"{saudacao} Podemos esclarecer toda a informação disponível sobre o histórico "
                "e manutenção desta viatura. Diga-nos concretamente o que pretende saber."
            )

        elif any(p in texto for p in ["contacto", "telefone", "telemóvel", "telemovel", "whatsapp", "número", "numero"]):
            resposta = f"{saudacao} Pode contactar-nos através do WhatsApp pelo número 962 148 367. Onde podemos ajudar?"

        else:
            resposta = f"{saudacao} Obrigado pelo seu contacto com a TC Car Premium. Onde podemos ajudar?"
